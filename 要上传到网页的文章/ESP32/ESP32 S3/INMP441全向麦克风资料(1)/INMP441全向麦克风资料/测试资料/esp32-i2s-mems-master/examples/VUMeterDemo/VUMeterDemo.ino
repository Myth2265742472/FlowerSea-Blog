/**
 * ESP32 I2S 音量表（VU表）示例
 *
 * 此示例基于 Adafruit I2S MEMS 扩展板上的"声压计"示例。
 * 除此之外的所有代码均属于公共领域。
 *
...
void loop() {
  ...
  if (samples_read > 0) {
    ...
    for (int i = 0; i < samples_read; ++i) {
      mean += (samples[i] >> 14);  // 右移 14 位以获得有效数据
    }
    mean /= samples_read;
    ...
    Serial.println(maxsample - minsample);  // 输出音量值（峰峰值）
  }
}
